diff --git a/ready_to_paste/custom_components/zendure_tempo/__init__.py b/ready_to_paste/custom_components/zendure_tempo/__init__.py
new file mode 100644
index 0000000000000000000000000000000000000000..c28f1aadb64fe837164df4e0ac7484b6df237f0d
--- /dev/null
+++ b/ready_to_paste/custom_components/zendure_tempo/__init__.py
@@ -0,0 +1,430 @@
...
+        vol.Optional("mode", default=MODE_AUTO): vol.In([MODE_AUTO, *ALL_MODES]),
+    }
+)
+
...
+
+    async def async_handle_apply_mode(call: ServiceCall) -> None:
+        """Apply the requested mode manually for testing."""
+        entry_id = call.data.get("config_entry_id")
+
+        target_coordinator: ZendureTempoCoordinator | None = None
+        if entry_id:
...
+            _LOGGER.error(
+                "Impossible d'appliquer le mode : aucune entrée cible trouvée (config_entry_id=%s)",
+                entry_id,
+            )
+            return
+
...
+        if mode == MODE_AUTO:
+            mode = target_coordinator.get_current_mode()
+
+        await target_coordinator._apply_mode(mode)
+        target_coordinator._last_mode = mode
+
+    if not hass.data[DOMAIN].get(SERVICE_REGISTERED):
+        hass.services.async_register(
+            DOMAIN,
+            SERVICE_APPLY_MODE,
+            async_handle_apply_mode,
+            schema=SERVICE_APPLY_MODE_SCHEMA,
+        )
...
+
+        remaining_entries = [
+            key for key in hass.data[DOMAIN].keys() if key != SERVICE_REGISTERED
+        ]
+        if not remaining_entries:
+            hass.services.async_remove(DOMAIN, SERVICE_APPLY_MODE)
